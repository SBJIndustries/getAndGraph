    ...
    for (let i = 0; i < urls.length; i++) {
        ...
        new Crawler().configure({
            ...
            .crawl(urls[i], function onSuccess(page) {
                if (page.url.search(urls[i].slice(11,urls[i].length-5))>0 && new RegExp(terms.join("|")).test(page.url)){// this just checks if main part of the urls is in the searched urls
